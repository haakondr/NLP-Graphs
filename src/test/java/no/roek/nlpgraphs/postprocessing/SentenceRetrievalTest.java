...
public class SentenceRetrievalTest {
	...
	public void shouldRetreieveSentences() {
		...
		PlagiarismReference ref2= new PlagiarismReference(filename, 6709, 277, "source-document06087.txt", 3157, 264);
		PlagiarismReference ref3= new PlagiarismReference(filename, 9615, 235, "source-document06087.txt", 2717, 236);
...
//			NLPSentence test = textPair.getTestSentence();
//			NLPSentence train = textPair.getTrainSentence();
//			if(SentenceUtils.isAlmostEqual(test.getStart(),6709) || SentenceUtils.isAlmostEqual(test.getStart(), 9615)) {
//				System.out.println(train.getStart());
//				System.out.println(test.getNumber()+": "+test.getText());
//				System.out.println(train.getNumber()+": "+train.getText());
//				
//			}
			if(textPair.getTestSentence().matchesPlagSuspiciousRef(ref)) {
				if(textPair.getTrainSentence().matchesPlagSourceRef(ref)) {
					return true;
				}
			}
		}
